...
WA Protocol adalah protokol lalu lintas tingkat Web4.0 pertama dengan kemampuan "propagasi sendiri, eksekusi sendiri, pembatasan sendiri".`}]}},ne=()=>{const s=[{feature:"AI智能分析",description:"基于深度学习的智能数据分析系统，提供实时洞察和预测"},{feature:"数据可视化",description:"直观的图表展示，支持多种数据格式和自定义样式"},{feature:"实时监控",description:"24/7全天候监控系统，及时发现异常并自动报警"},{feature:"云端同步",description:"多设备数据同步，随时随地访问和管理您的数据"}];return e.jsx("div",{className:"bg-white/5 border border-white/10 rounded-xl overflow-hidden backdrop-blur-sm",children:e.jsx("div",{className:"overflow-x-auto",children:e.jsxs("table",{className:"table-fixed w-full",children:[e.jsx("thead",{children:e.jsxs("tr",{className:"bg-white/5 border-b border-white/10",children:[e.jsx("th",{className:"w-1/3 px-6 py-4 text-left font-semibold text-white",children:"功能名称"}),e.jsx("th",{className:"w-2/3 px-6 py-4 text-left font-semibold text-white",children:"功能描述"})]})}),e.jsx("tbody",{children:s.map((m,p)=>e.jsxs("tr",{className:"hover:bg-white/5 transition-colors duration-200 border-b border-white/5 last:border-b-0",children:[e.jsx("td",{className:"px-6 py-4 text-slate-300 align-middle",children:e.jsx("span",{className:"inline-block text-sm leading-normal",children:m.feature})}),e.jsx("td",{className:"px-6 py-4 text-slate-300 align-middle",children:e.jsx("span",{className:"inline-block text-sm leading-normal",children:m.description})})]},p))})]})})})},ie=[{id:"architecture-diagram",title:"架构总览图（逻辑结构）",keywords:["架构总览图","逻辑结构"],description:"WA Protocol 操作系统架构总览图",component:null},{id:"flow-diagram",title:"流程结构图（逻辑结构）",keywords:["流程结构图","逻辑结构"],description:"WA Protocol 用户参与流程结构图",component:null},{id:"business-loop-diagram",title:"商业闭环总图（逻辑结构）",keywords:["商业闭环总图","逻辑结构"],description:"WA Protocol 商业闭环总图",component:null}],oe=s=>ie.find(m=>m.keywords.every(p=>s.includes(p)))||null;class k{static components=new Map;static register(m,p){this.components.set(m,p)}static get(m){return this.components.get(m)||null}static getAll(){return new Map(this.components)}}const x="chart-registry-updated",z={registerComponents(){M(()=>import("./OptimizedArchitectureImage-dkI9n79T.js"),__vite__mapDeps([0,1,2])).then(s=>{k.register("architecture-diagram",s.default),window.dispatchEvent(new Event(x))}),M(()=>import("./FlowDiagramImage-DSZUsU4x.js"),__vite__mapDeps([3,1,2])).then(s=>{k.register("flow-diagram",s.default),window.dispatchEvent(new Event(x))}),M(()=>import("./BusinessLoopImage-1oJ98ZHL.js"),__vite__mapDeps([4,1,2])).then(s=>{k.register("business-loop-diagram",s.default),window.dispatchEvent(new Event(x))})},renderChart(s,m,p){const f=oe(s);if(!f)return null;const g=k.get(f.id);return g?e.jsx("div",{className:"my-8",children:e.jsx(g,{...p})},`${f.id}-${m}`):null},getRegisteredCharts(){return k.getAll()},isChartRegistered(s){return k.get(s)!==null}},I=({text:s,index:m,language:p})=>{const[,f]=E.useState(0);return E.useEffect(()=>{const g=()=>f(T=>T+1);return window.addEventListener(x,g),()=>window.removeEventListener(x,g)},[]),z.renderChart(s,`chart-${m}`,{language:p})},re=()=>{E.useEffect(()=>{z.registerComponents()},[])};function se({language:s,setCurrentPage:m}){const[p,f]=w.useState(null),[g,T]=w.useState(null),P=ae[s],N=L[s]||L.en;re(),w.useEffect(()=>{const n=()=>{const i=N.sections;for(let l=i.length-1;l>=0;l--){const a=document.getElementById(`section-${i[l].id}`);if(a&&a.getBoundingClientRect().top<=80){f(i[l].id);break}}};return window.addEventListener("scroll",n,{passive:!0}),()=>window.removeEventListener("scroll",n)},[N.sections]),w.useEffect(()=>{window.scrollTo(0,0)},[]);const U=n=>{T(n);const i=()=>{const l=document.getElementById(`section-${n}`);if(!l)return;const a=document.querySelector("header"),u=a instanceof HTMLElement?a.getBoundingClientRect().height:0,h=l.getBoundingClientRect().top+window.scrollY-u-12;window.scrollTo({top:Math.max(0,h),behavior:"smooth"})};requestAnimationFrame(()=>{requestAnimationFrame(()=>{i(),requestAnimationFrame(()=>{i()})})})},G=n=>{T(g===n?null:n)},X=n=>{const i=n.split(`
`),l=[];let a=0;for(;a<i.length;){const u=i[a].trim();if(!u){a++;continue}if(u.startsWith("|")){let c=[];for(;a<i.length&&i[a].trim().startsWith("|");)c.push(i[a].trim()),a++;const h=c.filter(v=>v.trim());if(h.length>=2){const v=h[0].split("|").slice(1,-1).map(t=>t.trim()),o=v.length;let C=!1,D=0;const A=Array(Math.max(1,o)).fill(0);h.slice(2).forEach(t=>{t.split("|").slice(1,-1).forEach((b,d)=>{const W=b.trim().length;D=Math.max(D,W),A[d]=Math.max(A[d]??0,W),W>20&&(C=!0)})});const K=o<=3?22:14,j=!C&&D<=K,V="text-[13px]",q="text-[12px]",H=j?"whitespace-nowrap":"whitespace-normal",$=j?"whitespace-nowrap":"whitespace-normal",B=t=>{if(o===2){const r=A[0]||0,b=A[1]||0,d=Math.max(1,r+b),W=(r-b)/d,F=((Q,Z,ee)=>Math.min(ee,Math.max(Z,Q)))(42+W*12,38,46);return t===0?F:100-F}if(o===3){const r=A[1]||0,b=A[2]||0;return Math.abs(r-b)<=10?t===0?22:39:t===0?22:t===1?48:30}return o===4?t===0?15:t===1||t===2?25:35:o===5?t===0?12:t===1||t===2||t===3?20:28:o===6?t===0?10:18:o===7?t===0?8:t===1||t===2||t===3||t===4||t===5?15:17:t===0?25:75/Math.max(1,o-1)},y=o===2,_=C?"align-top":"align-middle",Y=t=>{const r=t.trim();if(!r)return!1;const b=r.replace(/,/g,"");return/^[-+]?\d+(?:\.\d+)?%?$/.test(b)},S=Array(Math.max(1,o)).fill(!1);for(let t=0;t<o;t++){const r=h.slice(2).map(d=>(d.split("|").slice(1,-1)[t]??"").trim()).filter(Boolean);if(r.length===0){S[t]=!1;continue}const b=r.filter(Y).length;S[t]=b/r.length>=.7}const J=t=>t===0?"text-left":S[t]?"text-right tabular-nums":y||o>=3||t===0?"text-left":(A[t]||0)<=8?"text-center":"text-left",R=t=>o===2?t===0?"pl-10 pr-7":"pl-7 pr-10":o===3?t===0?"pl-9 pr-7":t===1?"px-7":"pl-7 pr-9":o===4?t===0?"px-6":t===1||t===2?"px-4":"px-6":o>=5?t===0||t===o-1?"px-4":"px-3":"px-4";l.push(e.jsx("div",{className:"my-8",children:e.jsx("div",{className:"ui-panel-soft overflow-hidden",children:e.jsx("div",{className:"overflow-x-auto",children:e.jsx("div",{className:"px-4 sm:px-5 py-2",children:e.jsxs("table",{className:`table-auto ${y?"mx-auto w-full max-w-[560px]":o===3?"mx-auto w-full max-w-[720px] min-w-[620px]":"w-full min-w-[720px]"} border-0 border-separate`,style:{border:"none",borderCollapse:"separate",borderSpacing:"0"},children:[e.jsx("thead",{children:e.jsx("tr",{className:"bg-white/[0.06] border-b border-white/10",children:v.map((t,r)=>e.jsx("th",{style:{width:`${B(r)}%`},className:`${R(r)} ${y?"py-4":"py-3.5"} font-semibold text-slate-50 align-middle ${r===0?"text-left":S[r]?"text-right tabular-nums":"text-left"} ${r<o-1?"border-r border-white/10":""} ${!y&&o===3&&r===0?"whitespace-nowrap":$} ${!y&&o===3&&r===1?"whitespace-nowrap":""}`,children:e.jsx("span",{className:`block ${V} leading-5 tracking-wide text-slate-50/90`,children:t})},r))})}),e.jsx("tbody",{children:h.slice(2).map((t,r)=>e.jsx("tr",{className:"border-0 odd:bg-white/[0.015] hover:bg-white/[0.035] transition-colors",style:{border:"none"},children:t.split("|").slice(1,-1).map((b,d)=>e.jsx("td",{style:{width:`${B(d)}%`,border:"none"},className:`${R(d)} ${y?"py-4":"py-3.5"} text-slate-300 ${_} ${J(d)} ${!y&&o===3&&d===0?"whitespace-nowrap":H} ${!y&&o===3&&d===1?"whitespace-nowrap":""} border-0`,children:e.jsx("span",{className:`block ${q} leading-[22px] tracking-[0.01em] break-words ${d===0?"text-slate-100/90 font-medium":"text-slate-300/80"}`,children:b})},d))},r))})]})})})})},a))}continue}if(u.startsWith("•")){let c=[];for(;a<i.length&&i[a].trim().startsWith("•");)c.push(i[a].trim()),a++;l.push(e.jsx("ul",{className:"space-y-3",children:c.map((h,v)=>e.jsxs("li",{className:"flex items-start gap-3 text-slate-300 leading-relaxed break-words",children:[e.jsx("span",{className:"inline-flex items-center justify-center w-2 h-2 bg-lime-400 rounded-full mt-2 flex-shrink-0"}),e.jsx("span",{dangerouslySetInnerHTML:{__html:h.substring(1).trim().replace(/\*\*(.*?)\*\*/g,'<strong class="text-white font-medium">$1</strong>').replace(/#YouTube#/g,'<span class="inline-flex items-center px-2 py-1 mx-1 text-xs font-medium rounded-lg border" style="background-color: rgb(220 38 38); border-color: rgb(220 38 38); color: rgb(255 255 255);">YouTube</span>').replace(/#Facebook#/g,'<span class="inline-flex items-center px-2 py-1 mx-1 text-xs font-medium rounded-lg border" style="background-color: rgb(37 99 235); border-color: rgb(37 99 235); color: rgb(255 255 255);">Facebook</span>').replace(/#微博#/g,'<span class="inline-flex items-center px-2 py-1 mx-1 text-xs font-medium rounded-lg border" style="background-color: rgb(225 29 72); border-color: rgb(225 29 72); color: rgb(255 255 255);">微博</span>').replace(/#Weibo#/gi,'<span class="inline-flex items-center px-2 py-1 mx-1 text-xs font-medium rounded-lg border" style="background-color: rgb(225 29 72); border-color: rgb(225 29 72); color: rgb(255 255 255);">Weibo</span>').replace(/#门户网站#/g,'<span class="inline-flex items-center px-2 py-1 mx-1 text-xs font-medium rounded-lg border" style="background-color: rgb(245 158 11); border-color: rgb(245 158 11); color: rgb(0 0 0);">门户网站</span>').replace(/#BBS#/gi,'<span class="inline-flex items-center px-2 py-1 mx-1 text-xs font-medium rounded-lg border" style="background-color: rgb(5 150 105); border-color: rgb(5 150 105); color: rgb(255 255 255);">BBS</span>').replace(/#早期博客#/g,'<span class="inline-flex items-center px-2 py-1 mx-1 text-xs font-medium rounded-lg border" style="background-color: rgb(101 163 13); border-color: rgb(63 98 18); color: rgb(255 255 255);">早期博客</span>').replace(/#([^#]+)#/g,'<span class="inline-flex items-center px-2 py-1 mx-1 text-xs font-medium bg-lime-500/10 text-lime-300 rounded-lg border border-lime-500/25">$1</span>')}})]},v))},a))}else if(u.startsWith("✅"))l.push(e.jsx("h4",{className:"flex items-center gap-4 text-xl font-light text-emerald-400 mt-8 mb-6",dangerouslySetInnerHTML:{__html:u.replace(/\*\*(.*?)\*\*/g,'<strong class="text-white font-medium">$1</strong>').replace("✅",'<span class="inline-flex items-center justify-center w-7 h-7 bg-emerald-500 text-white text-lg rounded-xl font-bold">✓</span>')}},a)),a++;else if(u.startsWith("**")&&u.endsWith("**")){const c=u.slice(2,-2);l.push(e.jsx("h3",{className:"text-xl font-light text-lime-400 mt-8 mb-6",children:c},a)),c==="架构总览图（逻辑结构）"&&l.push(e.jsx("div",{className:"my-8",children:e.jsx(ne,{})},`demo-${a}`)),l.push(e.jsx(I,{text:c,index:a,language:s},`chart-${a}`)),a++}else if(u.startsWith("###")){const c=u.replace("###","").trim(),h=c.startsWith("**")&&c.endsWith("**")?c.slice(2,-2):c;l.push(e.jsx("h3",{className:"text-xl font-light text-lime-400 mt-8 mb-6",children:h},a)),l.push(e.jsx(I,{text:h,index:a,language:s},`chart-${a}`)),a++}else if(u.startsWith("*")&&u.endsWith("*")){const c=u.slice(1,-1).trim();l.push(e.jsx("h3",{className:"text-xl font-light text-lime-400 mt-8 mb-6",children:c},a)),l.push(e.jsx(I,{text:c,index:a,language:s},`chart-${a}`)),a++}else if(u.startsWith(">"))l.push(e.jsx("blockquote",{className:"border-l-4 border-lime-500 pl-6 py-4 my-6 bg-lime-500/10 rounded-r-xl text-slate-300 italic leading-relaxed backdrop-blur-sm break-words",children:u.replace(">","").trim()},a)),a++;else if(u.startsWith("---"))l.push(e.jsx("div",{className:"my-12",children:e.jsx("div",{className:"border-t border-white/10"})},a)),a++;else{let c=u;for(;a+1<i.length&&i[a+1].trim()&&!i[a+1].trim().startsWith("|")&&!i[a+1].trim().startsWith("•")&&!i[a+1].trim().startsWith("**")&&!i[a+1].trim().startsWith("###")&&!i[a+1].trim().startsWith("*")&&!i[a+1].trim().startsWith("✅")&&!i[a+1].trim().startsWith(">")&&!i[a+1].trim().startsWith("---");)a++,c+=" "+i[a].trim();l.push(e.jsx("p",{className:"text-slate-300 leading-relaxed mb-4 break-words",dangerouslySetInnerHTML:{__html:c.replace(/\*(.*?)\*/g,'<strong class="text-white font-medium">$1</strong>').replace(/\*\*(.*?)\*\*/g,'<strong class="text-white font-medium">$1</strong>')}},a)),a++}}return e.jsx(e.Fragment,{children:l})};return e.jsxs("div",{className:"min-h-screen bg-[#0a0a0a] text-white",children:[e.jsxs("div",{className:"fixed inset-0 overflow-hidden z-[-1] opacity-60",children:[e.jsx("div",{className:"absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[1200px] h-[800px] bg-gradient-to-br from-lime-500/20 via-emerald-500/10 to-cyan-500/20 rounded-full blur-3xl animate-pulse"}),e.jsx("div",{className:"absolute top-1/3 left-1/3 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[600px] bg-gradient-to-tr from-blue-500/20 via-indigo-500/10 to-purple-500/20 rounded-full blur-3xl animate-pulse delay-700"}),e.jsx("div",{className:"absolute bottom-1/3 right-1/3 -translate-x-1/2 -translate-y-1/2 w-[1000px] h-[700px] bg-gradient-to-bl from-amber-500/20 via-orange-500/10 to-red-500/20 rounded-full blur-3xl animate-pulse delay-1400"})]}),e.jsxs("div",{className:"container mx-auto px-6 py-20 relative max-w-2xl lg:max-w-3xl",children:[e.jsx(O,{className:"absolute top-28 right-[15%] text-lime-400/40",size:16}),e.jsx(O,{className:"absolute top-48 left-[20%] text-lime-300/30",size:12}),e.jsx(te,{className:"absolute bottom-32 right-[25%] text-lime-400/30",size:18}),e.jsxs("header",{className:"mb-24 text-center",children:[e.jsxs("div",{className:"inline-flex items-center gap-2 px-4 py-1.5 mb-8 text-xs tracking-wide text-lime-300/90 bg-lime-500/5 border border-lime-500/20 rounded-full",children:[e.jsx("span",{className:"w-1.5 h-1.5 bg-lime-400 rounded-full animate-pulse"}),"Whitepaper Documentation"]}),e.jsx("h1",{className:"mb-8",children:e.jsx("div",{className:"text-4xl md:text-5xl lg:text-6xl text-white font-light tracking-tight mb-3",children:P.title})}),e.jsx("p",{className:"text-slate-400 text-lg mb-14 max-w-2xl mx-auto font-light leading-relaxed",children:P.subtitle})]}),e.jsx("nav",{className:"mb-24",children:e.jsxs("div",{className:"ui-panel-soft ui-panel-hover p-8 hover:border-emerald-500/50 transition-all duration-300",children:[e.jsxs("h2",{className:"text-xl font-light text-white mb-8 flex items-center gap-3",children:[e.jsx("span",{className:"w-2 h-2 bg-lime-400 rounded-full animate-pulse"}),P.tableOfContents]}),e.jsx("div",{className:"grid gap-3",children:N.sections.map(n=>e.jsx("button",{onClick:()=>U(n.id),className:`group text-left p-5 rounded-xl transition-all duration-300 ${p===n.id?"bg-lime-600/20 border border-lime-500/30 text-lime-400 shadow-lg shadow-lime-500/10":"hover:bg-white/5 border border-transparent text-slate-300 hover:text-white hover:border-white/20"}`,children:e.jsxs("div",{className:"flex items-center justify-between",children:[e.jsxs("div",{className:"flex items-center gap-4",children:[e.jsx("div",{className:`w-10 h-10 rounded-lg flex items-center justify-center transition-all duration-300 font-mono text-sm ${p===n.id?"bg-lime-600 text-white shadow-lg":"bg-white/10 text-slate-500 group-hover:bg-lime-600/20 group-hover:text-lime-400"}`,children:String(n.id).padStart(2,"0")}),e.jsxs("div",{children:[e.jsx("div",{className:`font-medium transition-colors duration-300 ${p===n.id?"text-lime-300":"text-white group-hover:text-lime-400"}`,children:n.title}),e.jsxs("div",{className:"text-xs text-slate-500 mt-1",children:["Chapter ",n.id]})]})]}),e.jsx("div",{className:`transition-all duration-300 ${p===n.id?"text-lime-400":"text-slate-600 group-hover:text-lime-400"}`,children:e.jsx("div",{className:"w-2 h-2 rounded-full bg-current"})})]})},n.id))})]})}),e.jsx("main",{className:"space-y-12",children:N.sections.map(n=>e.jsxs("article",{id:`section-${n.id}`,className:"ui-panel-soft ui-panel-hover group hover:border-emerald-500/50 transition-all duration-300 hover:-translate-y-1 overflow-hidden",children:[e.jsx("button",{onClick:()=>G(n.id),className:"w-full text-left p-8 hover:bg-white/5 transition-colors duration-300",children:e.jsxs("div",{className:"flex items-center justify-between",children:[e.jsxs("div",{className:"flex items-center gap-6",children:[e.jsx("div",{className:`w-14 h-14 rounded-xl flex items-center justify-center transition-all duration-300 font-mono text-lg font-bold ${g===n.id?"bg-lime-600 text-white shadow-lg shadow-lime-500/30":"bg-white/10 text-slate-400 group-hover:bg-lime-600/20 group-hover:text-lime-400"}`,children:String(n.id).padStart(2,"0")}),e.jsxs("div",{children:[e.jsx("h2",{className:`text-xl md:text-2xl font-light transition-colors duration-300 ${g===n.id?"text-lime-400":"text-white group-hover:text-lime-400"}`,children:n.title}),e.jsx("div",{className:"text-sm text-slate-500 mt-2",children:g===n.id?"Click to collapse":"Click to expand"})]})]}),e.jsx("div",{className:`flex items-center justify-center w-12 h-12 rounded-xl transition-all duration-300 ${g===n.id?"bg-lime-600 text-white rotate-45":"bg-white/10 text-slate-400 group-hover:bg-lime-600/20 group-hover:text-lime-400"}`,children:e.jsx("span",{className:"text-2xl font-light",children:g===n.id?"−":"+"})})]})}),g===n.id&&e.jsx("div",{className:"border-t border-white/10 bg-black/20",children:e.jsx("div",{className:"p-8",children:X(n.content)})})]},n.id))}),e.jsx("div",{className:"mt-24 text-center",children:e.jsxs("button",{onClick:()=>m("home"),className:"group px-8 py-4 text-sm font-medium text-white bg-lime-600 hover:bg-lime-500 rounded-xl transition-all duration-300 hover:shadow-xl hover:shadow-lime-500/25 flex items-center justify-center mx-auto gap-3 hover:-translate-y-1",children:[e.jsx("span",{className:"transition-transform duration-300 group-hover:-translate-x-1",children:"←"}),P.backToHome]})})]})]})}const de=w.memo(se);export{de as WhitepaperPage};
